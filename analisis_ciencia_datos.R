...
#DE ESTO ES LO QUE QUIERO HACER UNA REDUCCION DE DIMENSIONALIDAD 
...
library(psych)
pcarot <- principal(data4, nfactors=5, rotate="varimax",scores=T) 
...
data_comp <- pcarot$scores

# Hierarchical Clustering - Complete linkage, Ward, Single linkage
d <- dist(data_comp, method = "euclidean") # distance matrix --> ward
...
# get cluster means 
write.table(file="cluster_datos1",aggregate(data4,by=list(groups),FUN=mean),sep=",",dec=".")
